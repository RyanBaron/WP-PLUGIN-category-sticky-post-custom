...
class Category_Sticky_Post {
    ...
    if( $query->is_main_query() && is_archive() && 0 == get_query_var( 'paged' ) && '' != get_query_var( 'cat' ) ) {
      ...
      $sticky_posts = isset( $sticky_query_secondary->posts ) && ! empty( $sticky_query_secondary->posts ) ? $sticky_query_secondary->posts : array();
      $sticky_post_count = count( $sticky_posts );

      // Reset the main query.
      wp_reset_postdata();

      // If we dont have at least sticky posts than only highlight the primary (above) and ignore the secondary.
      if( $sticky_post_count >= 4 ) :
        $secondary_post_ids = array_slice($sticky_posts, 1, 4);   // return the secondary posts (skipping the first (primary) post)

        // Loop through the
        foreach ($secondary_post_ids as $key => $post_id ) {
          // Store the secondary feature sticky post in an array
      ...
      $sticky_query = $this->get_sticky_feature_primary_query( $category );
  ...
  /**
	 ...
	 * @since    2.0.0
	 */
	private function get_sticky_feature_primary_query( $category ) {
	 ...
	 * @since    2.0.0
	 */
	private function get_sticky_feature_secondary_query( $category ) {
